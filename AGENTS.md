...
- 文本文件使用 CRLF 换行符；`.cmd` 文件必须使用 CRLF，`.sh` 文件遵循 `.gitattributes` 使用 LF。
- 代码文件使用 Tab 字符缩进，代码中不允许连续两个及以上空行。`SUMMARY.md` 使用空格缩进导航层级。
- 不要修改与当前任务无关的文件，不要重排整篇文档，不要批量替换风格差异。
- 本仓库是独立的中文文档库，不适用其他项目的双语 README 约定。根目录及专题目录均只保留中文内容的 `README.md`，不创建其他语言版本的 README，也不添加语言切换栏；其他正文使用简体中文。术语、包名、类型名、命令名和文件名保持原文。
- `SUMMARY.md` 和库内链接指向对应的 `README.md`；引用其他仓库的双语文档时，仍保留目标仓库实际使用的文件名，不因本库约定而修改外部链接。
