...
**服务提供者**按连接名称供应缓存、锁等实例；**可匹配服务**按名称或格式找到求值器、归档器；**插件树构件**通过扩展路径挂载文件系统、[工作器](https://github.com/Zongsoft/framework/blob/main/Zongsoft.Core/src/Components/IWorker.cs)及处理器集合。它们都属于插件扩展，但获取方式不同。
