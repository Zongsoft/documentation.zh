...
启动即退出时，检查入口、运行时、程序集及配置加载错误。进程存活但不工作时，检查[工作器](https://github.com/Zongsoft/framework/blob/main/Zongsoft.Core/src/Components/IWorker.cs)是否挂载、启动是否失败、消息订阅或作业存储是否正常。仅在服务环境出错时，重点比较工作目录、账号和环境配置。
